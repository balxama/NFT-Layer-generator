...
function image_maker($text1,$amount,$count,$i){
    ...
    imagettftext($image, 16, 0, 980,50, $text_color,$font, "ATTACKS: ".strtoupper($count));
    //imagettftext($image, 20, 0, 1400,120, $text_color,$font, "Amount: ".$counter);
    imagettftext($image, 16, 0, 1700,50, $text_color,$font,"TOTAL: $ ".strtoupper($amount));
    //header("Content-Type: image/png");
    header( "Content-type: image/png" );
   // imagepng($image);
   if (!file_exists("bots/info_layer_$i")) {
    mkdir("bots/info_layer_$i", 0777, true);
}
    $save = "bots/info_layer_$i/".strtolower("info_layer_$i") .".png";
    imagepng($image, $save);
    //imagedestroy($image);
}
?>