...
$db=getDB();
$array=array();
$statement=$db->prepare("SELECT * from sheet1 ORDER BY amount DESC LIMIT 1");
$statement->execute();
...
function image_maker($number,$amount,$count,$i){
    ...
    imagettftext($image, 16, 360,1950,750, $text_color,$font, strtoupper($number));
    imagettftext($image, 16, 90, 1950,1250, $text_color,$font,"TOTAL: $ " . strtoupper($amount));
    //imagettftext($image, 20, 0, 1400,120, $text_color,$font, "Amount: ".$counter);
    imagettftext($image, 16, 90, 1950,1700, $text_color,$font, "ATTACKS: ".strtoupper($count));

    //header("Content-Type: image/png");
    header( "Content-type: image/png" );
   // imagepng($image);
   if (!file_exists("bot4/info_layer_$i")) {
    mkdir("bot4/info_layer_$i", 0777, true);
}
    $save = "bot4/info_layer_$i/".strtolower("info_layer_$i") .".png";
    imagepng($image, $save);
}
