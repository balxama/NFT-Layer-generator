...
function image_maker($text1,$text,$date,$protocol,$i){
    ...
    imagettftext($image, 16, 0, 1250,50, $text_color,$font, "DATE: ".$date);
    imagettftext($image, 16, 0, 1700,50, $text_color,$font, "PROTOCOL: ".strtoupper($protocol));
    //header("Content-Type: image/png");
    header( "Content-type: image/png" );
   // imagepng($image);
   if (!file_exists("sandwich_horizontal/info_layer_$i")) {
    mkdir("sandwich_horizontal/info_layer_$i", 0777, true);
}
    $save = "sandwich_horizontal/info_layer_$i/".strtolower("info_layer_$i") .".png";
    imagepng($image, $save);
    //imagedestroy($image);
}
?>